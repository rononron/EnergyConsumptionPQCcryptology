...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        int verificationResult = crypto_sign_verify(signature, signatureLength, (const uint8_t *)message, messageLength, pk);

        if (verificationResult == 0) {
            printf("[SIG %i] Die Signatur ist gültig.\n", i);
        } else {
            printf("Die Signatur ist ungültig.\n");
        }
